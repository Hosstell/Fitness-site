...
class MainMutation(graphene.Mutation):
    class Meta:
        abstract = True
    ...
    def __init_subclass_with_meta__(cls, output=None, input_fields=None, arguments=None, name=None, **options):
        ...
        if cls.mutate and cls.mutate.__func__ == MainMutation.mutate.__func__:
            assert mutate_and_get_payload, (
                "{name}.mutate_and_get_payload method is required"
                " in a MainMutation.").format(name=name or cls.__name__)

        if not name:
            name = '{}Payload'.format(base_name)

        super(MainMutation, cls).__init_subclass_with_meta__(
